<?php
 $user = current_user();
 if (isset($_POST["reviewed_gs_utf"])) {
    $Search = $_GET["id"];

    if (!empty($Search)) {
        $search_data = find_by_sql(
            "SELECT * FROM grain_size_upstream_transition_fill WHERE id = '{$Search}' LIMIT 1"
        );
        ...
        if ($search_data) {
            ...
            if (!$existing_record) {
                ...
                if ($db->query($sql)) {
                    ...
                    redirect("../reviews/grain-size-upstream-transition-fill.php?id=" . $Search, false);
                } else {
                }
            } else {
