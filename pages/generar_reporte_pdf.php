...
// Validar fecha
if (!isset($_GET['fecha'])) die('Fecha no especificada.');
...
$follow_tables = ['test_preparation','test_delivery','test_realization','test_repeat','test_review','test_reviewed'];
...
foreach ($requisitions as $req) {
  for ($i = 1; $i <= 20; $i++) {
    ...
    if (!isset($indexed_status[$key])) {
      if (!isset($pending_by_type[$test_type])) {
        $pending_by_type[$test_type] = 0;
      }
      $pending_by_type[$test_type]++;
    }
  }
}
...
foreach ($pending_by_type as $type => $count) {
  $pdf->Cell(100, 8, $type, 1, 0);
