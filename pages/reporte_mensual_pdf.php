...
function ensure_graph_space($pdf,$height){
...
$x = $chartX+10;
$max = empty($trend)?1:max(array_column($trend,'total'));

foreach($trend as $i=>$t){
    $h = ($t['total']/$max)*$chartH;
    $y = $chartY + ($chartH - $h);

    $pdf->SetFillColor(40,90,200);
...
/* ======================================================
   9. CRITICAL TESTS SUMMARY (WITH GRAPH)
====================================================== */

$pdf->section_title("6. Critical Tests Summary");

$critical_list = ['SP','HY','SND','UCS','LAA','SG','GS','AR'];

$completed = tests_completed_by_type($start_str,$end_str);
$registered = tests_registered_by_type($start_str,$end_str);

$regMap=[];  foreach($registered as $r){ $regMap[strtoupper($r['Test_Type'])]=$r['total']; }
$compMap=[]; foreach($completed as $c){ $compMap[strtoupper($c['Test_Type'])]=$c['total']; }
...
foreach($critical_list as $t){
    ...
    $p = $r - $c;
    if($r>0 || $c>0){
        $critical_rows[]=['type'=>$t,'reg'=>$r,'comp'=>$c,'pend'=>$p];
    }
}

ensure_graph_space($pdf,90);
...
$maxVal = empty($critical_rows)?1:max(array_column($critical_rows,'comp'));

foreach($critical_rows as $r){
    $h = ($r['comp'] / $maxVal) * ($chartH - 5);
    $y = $chartY + ($chartH - $h);

    $pdf->SetFillColor(40,160,90);
...
/* Mapeo */
$regMap  = []; foreach($reg_type as $r){ $regMap[strtoupper($r['Test_Type'])]=$r['total']; }
$compMap = []; foreach($comp_type as $c){ $compMap[strtoupper($c['Test_Type'])]=$c['total']; }
...
$values = array_map(fn($t)=>$regMap[$t] ?? 0 , $allTypes);
$maxVal = empty($values)?1:max($values);
