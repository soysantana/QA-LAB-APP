...
// BASE URL relativo a /pages
$BASE_URL = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/'); // p.ej. /mi_app/public/pages
...
$q = trim($_GET['q'] ?? '');
if ($q !== '') {
  // IMPORTANTE: escapar para LIKE (usar %...%)
  $q_esc = $db->escape($q);
  $like  = "%{$q_esc}%";
  $sql = "
    SELECT * FROM doc_files
     WHERE
       COALESCE(sample_id,'')      LIKE '{$like}' OR
       COALESCE(sample_number,'')  LIKE '{$like}' OR
       COALESCE(test_type,'')      LIKE '{$like}' OR
       COALESCE(template,'')       LIKE '{$like}' OR
       COALESCE(file_name,'')      LIKE '{$like}' OR
       COALESCE(file_path,'')      LIKE '{$like}' OR
       COALESCE(status,'')         LIKE '{$like}'
...
// Carga inicial (filtrada o completa)
...
<main id="main" class="main">
  ...
  <div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="<?= e($BASE_URL) ?>/home.php">Home</a></li>
        ...
        <div class="card">
          <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div class="d-flex align-items-center gap-2">
              ...
              <h5 class="mb-0">Últimos documentos generados / subidos</h5>
              <?php if ($q !== ''): ?>
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <form class="d-flex align-items-center" method="get" action="">
                <div class="input-group input-group-sm">
                  <input
                    ...
                    name="q"
                    value="<?= e($q) ?>"
                    placeholder="Buscar por sample, test, template, archivo..."
                    aria-label="Buscar"
                  >
              ...
              <a href="<?= e($BASE_URL) ?>/doc_upload_external.php" class="btn btn-primary btn-sm">
            ...
            <div class="table-responsive">
              <table class="table table-sm table-hover align-middle">
                <thead class="table-light">
                  <tr>
                    <th style="min-width:140px"><i class="bi bi-clock"></i> Fecha</th>
                    <th style="min-width:220px"><i class="bi bi-hash"></i> Sample</th>
                    <th style="min-width:80px">Test</th>
                    <th style="min-width:160px">Template</th>
                  ...
                  <?php if (empty($rows)): ?>
                    <tr>
                      <td colspan="7" class="text-center text-muted py-4">
                        ...
                        <?php if ($q !== ''): ?>
                          Sin resultados para <strong><?= e($q) ?></strong>.
                  ...
                  <?php else: ?>
                    <?php foreach ($rows as $r): ?>
                      <?php
                        ...
                        $statusText = ($r['status'] === 'signed') ? 'Firmado' : 'En espera de firma';
                        ...
                        <td>
                          <div class="d-flex flex-wrap gap-2">
                            <?php if (!empty($file_path)): ?>
                              <!-- Modal interactivo: colocar textos -->
                              <a class="btn btn-outline-primary btn-sm btn-colocar"
                                 data-bs-toggle="modal"
                                 data-bs-target="#pdfPlaceModal"
                                 data-doc-id="<?= (int)$r['id'] ?>"
                                 data-view-url="<?= e($BASE_URL) ?>/serve_pdf.php?<?= http_build_query(['id'=> (int)$r['id']]) ?>"
                                 data-file-name="<?= e($file_name) ?>">
                                Colocar textos
                              </a>

                              <!-- Ver PDF (antes y después de firmar) -->
                              <a class="btn btn-outline-secondary btn-sm"
                                 href="<?= e($BASE_URL) ?>/serve_pdf.php?<?= http_build_query(['id'=> (int)$r['id']]) ?>"
                            ...
                            <?php if (($r['status'] ?? '') !== 'signed'): ?>
                              <form action="<?= e($BASE_URL) ?>/doc_mark_signed.php" method="post" class="m-0">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title d-flex align-items-center gap-2" id="pdfPlaceLabel">
            ...
            <span id="pdfPlaceTitle">Colocar textos en PDF</span>
          </h5>
  ...
  <script type="module">
    // PDF.js (ESM) + worker (misma versión)
    ...
    let marks=[]; // { pageIndex, xCss, yCss, viewportWidthCss, kind }
    let rendering=false;
    ...
    async function insert(mode){
      try{
        ...
        const base=await fetch(currentUrl).then(r=>{ if(!r.ok) throw new Error('No se pudo cargar ('+r.status+')'); return r.arrayBuffer(); });
        const pdfDoc=await PDFDocument.load(base);
        const font=await pdfDoc.embedFont('Helvetica'); const size=10;

        const byPage=new Map(); for(const m of marks){ if(!byPage.has(m.pageIndex)) byPage.set(m.pageIndex,[]); byPage.get(m.pageIndex).push(m); }
        for(const [idx,arr] of byPage.entries()){
          const page=pdfDoc.getPage(idx); const {width:pw,height:ph}=page.getSize();
          for(const m of arr){ const val=data[m.kind]||''; const {xPdf,yPdf}=cssToPdfPoint(m.xCss,m.yCss,m.viewportWidthCss,pw,ph);
        ...
        if(mode==='download'){
          ...
          a.href=URL.createObjectURL(blob); a.download=(($('fileName2')?.value||'document').replace(/\.pdf$/i,''))+'_text_'+Date.now()+'.pdf';
          a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),1000); msg.textContent='Descargado ✅'; return;
        }

        // Subir
        msg.textContent='Subiendo…';
        const fd=new FormData(); const baseName=(($('fileName2')?.value||'document').replace(/\.pdf$/i,''))+'_text_'+Date.now()+'.pdf';
        fd.append('file', new Blob([out],{type:'application/pdf'}), baseName);
        const id=$('docId2')?.value; if(id) fd.append('doc_id', String(id));
        // OJO: ruta relativa según tu estructura; aquí vas a tu endpoint real:
        const resp=await fetch('../database/upload_pdf_editado.php',{method:'POST', body:fd});
        const txt=await resp.text(); if(!resp.ok) throw new Error(txt||'Error al subir');
    ...
    document.querySelectorAll('.btn-colocar').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        ...
        try{
          const head=await fetch(currentUrl);
          if(!head.ok){ msg.textContent='No se pudo cargar (status '+head.status+')'; return; }
          const ct=(head.headers.get('Content-Type')||'').toLowerCase();
          if(!ct.includes('application/pdf')){ msg.textContent='La respuesta no es PDF (Content-Type: '+ct+')'; return; }
        }catch(err){ msg.textContent='Error de red: '+err.message; return; }
